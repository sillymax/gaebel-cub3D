...
/*   Updated: 2024/08/07 17:28:43 by ychng            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_left_right(t_main *main, int side_direction)
{
	...
	angle_offset = PI / 2;
	new_x = main->player.center_x + \
	cos(main->player.rotation_angle + angle_offset) * velocity;
	new_y = main->player.center_y + \
	sin(main->player.rotation_angle + angle_offset) * velocity;
	if (!has_wall_at(main, new_x, new_y))
	{
		main->player.center_x = new_x;
		main->player.center_y = new_y;
